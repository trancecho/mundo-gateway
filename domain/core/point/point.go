...
var (
	...
	pointServerAddr = "localhost:50051" // 可通过配置文件或环境变量设置
	pointTimeout    = 10 * time.Second  // 超时设置
)

// InitPointClient 初始化积分服务客户端连接
func InitPointClient() error {
	var err error
	...
	pointClientOnce.Do(func() {
		if serverAddr != "" {
			pointServerAddr = serverAddr
		}

		// 从地址中去除 grpcs:// 前缀（如果存在）
		address := pointServerAddr
		...
		if err == nil {
			pointClient = point.NewUserServiceClient(pointConn)
		}
	})
	return err
}
